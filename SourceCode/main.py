...
def add_department(session: Session):
    unique: bool = False
    constrants: list = []
    constraintVals: list = ["name", "abbreviation", "chair name", "building and office", "description"]
    name: str = ''
    abbreviation: str = ''
    chairName: str = ''
    building: str = ''
    ...
    while not unique:
        ...
        constrants.append(session.query(Department).filter(Department.name == name).count())
        constrants.append(session.query(Department).filter(Department.abbreviation == abbreviation).count())
        constrants.append(session.query(Department).filter(Department.chairName == chairName).count())
        constrants.append(session.query(Department).filter(Department.building == building,
                                                           Department.office == office).count())
        constrants.append(session.query(Department).filter(Department.description == description).count())

        unique = True
        for c in range(0,5):
            if constrants[c] > 0:
                unique = False
                print("We already have that department " + constraintVals[c] + " in our system.")
...
def add_student_section(sess):
    ...
    unique_student_major: bool = student_section_count == 0
    while not unique_student_major:
...
def add_section_student(sess):
    ...
    sess.add(section)                           # add the Enrollment to the session
    sess.flush()
...
def select_student(sess) -> Student:
    ...
    while not found:
        ...
        count: int = sess.query(Student).filter(Student.studentID == id).count()
        if count == 0:
            print("No student found by that name.  Try again.")
        else:
            found = True
    student: Student = sess.query(Student).filter(Student.studentID == id).first()
    return student
...
def delete_course(session):
    ...
    n_sessions = sess.query(Section).filter(Section.departmentAbbreviation == course.departmentAbbreviation,
                                            Section.courseNumber == course.courseNumber).count()
    if n_sessions > 0:
        print(f"Sorry, there are {n_sessions} sections in that course.  Delete them first, "
              "then come back here to delete the course.")
    else:
        session.delete(course)


def delete_major(session):
    print("deleting a major")
    section: Section = select_major(session)
...
def list_student_section(sess):
    student: Student = select_student(sess)
    recs = sess.query(Student).join(Enrollment, Student.studentID == Enrollment.studentID).join(
        Section, Enrollment.sectionNumber == Section.sectionNumber).filter(
        Student.firstName == student.firstName,
        Student.lastName == student.lastName).add_columns(
        Section.sectionNumber, Section.courseNumber, Section.departmentAbbreviation).all()
    for stu in recs:
        print(f"Section: {stu.sectionNumber}, Course number: {stu.courseNumber}, Department: {stu.departmentAbbreviation}")


def list_section_student(sess: Session):
        section: Section = select_section(sess)
        sects = sess.query(Section).join(Enrollment, Enrollment.sectionNumber == Section.sectionNumber).join(
            Student, Enrollment.studentID == Student.studentID).filter(
                ...
                Section.semester == section.semester
                    ).add_columns(Student.lastName, Student.firstName).all()
        print("Students in section:")
        for stu in sects:
            print(f"Student: {stu.firstName} {stu.lastName}.")
        print("\n")
